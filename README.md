...
よくわからないので実験　→ 1,連続区間を伸ばしたい 2,操作によって端を連続にすることはできるが、それ以外の連続性は変化しない(ここまで明確に意識できたわけではなかったが)  ex) LR LRRL RL  → LR RLLR RL とすれば操作対象区間の端点の向きが前後で一致し連続になる。 →　このことから、Lを操作対象に考え前から順にLからRに変化する場所を K回見るまで探索しながらそれまでの長さを測定すればよいと考えた。 

Lを操作する場合にLが末尾の場合に困るので L から R に変化する場所の回数を測定しているこｔ、L始まりの場合にそこでは操作しないようにすること、Rが連続している場所の処理など、様々な工夫によってギリギリのバランスでACすることができている。
