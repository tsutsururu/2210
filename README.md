...
1, 最大値を記憶しておき、Aiが最大値ならば降順に並べたリストの1番目を、そうでなければ0番目を出力

2, heapq で管理し、Aiが最大値ならばpop → 先頭を出力 →  push 。そうでなければ先頭を出力する 

3, Ai-1までの最大値 と Ai+1以降の最大値を比較

i 番目を除いた最大値とは i-1番目以前とi+1番目以降の最大値になることに注目。これをO(1)で出力できるデータ構造を作ることができれば解答できる。→ i-1番目以降の最大値は 普通にそれまでの最大値を格納していれば X[i-1] で出力できる。問題はi+1番目以降の最大値である。これは後ろから探索した最大値を格納することで実現できる。新たな最大値を前に前に格納することになるので、que にappedleftしていけば 煩雑なindex の処理を行わずに実装できる。

なお、この解法が塁積和と表現されているが意味が分からない
...
# 121C Energy Drink Collector  diff 360
...
A　を昇順に探索してBを累積する処理を、Mになるまで貪欲に行えばよいと判断 → AC


# 171E Red Scarf  diff  778
...
実験　→ サンプル1 の状況を書き出してみると X2^X3^X4 = A1 , X1^X3^X4 = A2 , X1^X2^X4 = A3 , X1^X2^X3=A4 となる。ここで A^B=C が成立するなら A=B^C が成立する移項関係を知っていたので X3^X4=A1^X2,X3^X4=A2^X1 を導き、A1^A2=X1^X2 が得られた。ここで X1^X2^X4=A3 なので、X4=A1^A2^A3 が成立。各 X,A の関係は対称だからXi は Aiを除いた排他的論理和で表現できると判断　→ このままでは O(N^2)になりそうなので あらかじめすべての排他的論理 Aall を求めて、 Ai^Xi=All の関係から Xi を All^Ai で表現することで O(N)で実装できるようになって完了 → AC

N　が偶数の条件はこの問題を成立させるために何かしらの形で効いてるのだろうと判断してよく考えず提出した。

# 146C Buy an Integer  diff 741 
...
桁数で全探索してその桁数の最大値を買えないなら、どこまで買えるか (X - B* 桁数) // A で求めようと考えた。しかしサンプルがうまく通せないので視点を変えると、単調性から二分探索できるとわかってこれで解答した。なお break 処理を間違えたり、すべての整数を買える場合の処理などをミスりちらかした

➁　解法

桁数で探索しなくとも、全範囲で単調性が存在するので、mid の桁数を求めれば全範囲での二分探索ができる。

③ 初期発想補足

最大値を買えない場合を正しく表現できていなかった。あとは、なにも買えないとき (X - B* 桁数)//A が非正整数になるので max(0,)処理が必要。修正すると解答できた
...
素数列挙を素直に行うと O(N√N) になるが、i ～ N までの整数に対して iで割り切れるものをチェックしていき、チェックされていない整数を列挙することで素数を列挙すれば、計算量は  N/2 + N/3 + N/5 + ... になって、O(NloglogN) と信じられない速度で処理できる。

* 素数でないものに関しても倍数列挙するれば NlogN の調和級数オーダーになる。これでも十分高速
...
minfactor　配列を作成し、minfactor[x]に xの約数である最大の素数を、エラトステネスの篩によって格納していく。作成し終えたら、Counter Z を作成し、Z[minfactor[N]]+=1 , N//=minfactor[N] を N が1になるまで繰り返すことで素因数分解を実現できる。

O(logN)になるらしい。素直な素因数分解よりはるかに高速
...
くそめんどくさいが、1,エラトステネスの篩で100までの素数列挙  2, 素直に 2,3,5の倍数の個数をカウント　→ これらを 100 から引いて 解答　→ WA → 1を引き忘れていたことに気づいて修正 AC
...
エラトステネスの篩で N以下の素数を列挙した後、得られた素数を前から全探索して条件を満たす個数をカウントしていけばよいと考えた。N以下の素数の数は明らかにN以下なので、O(N+NloglogN) を超えることはないので十分高速だと判断


# 1031 

# 051B Sum of Three Integers  diff 784
...
x,y,z の全探索はできないが、x,yを全探索して 条件を満たす z が存在すればカウントをインクリメントする処理で O( 2500^2) だから十分高速と判断
...
N=10 なので普通に友達の友達を全探索できると判断　→ 一度数えた友達の友達を重複して数えないように、友達の友達を監理する必要があることに気づいてAC


➁　補足

N がもっと大きいような全探索ができないような場合 ( 例,アルゴ式 グラフアルゴリズム 1-4 友達の友達 ) を考える。グラフを考え、bfsで頂点0 からの最短距離を求めると、友達の友達とは (自分の距離 + 2) or (自分の距離 -2) の距離を持つものだと解釈できる。(　距離 0 なら 距離2 のみが、距離 1 なら 距離3 と 自分自身を除く距離1 が該当する) あとはカウンターで距離を管理すれば O(M)で処理できると判断した。→ この距離の関係が成立するのはグラフが木の場合に限る。サイクルの存在で崩壊するのだ。この方法では結局、始点ごとに距離を求めなおさなければならない。グラフ次第では距離2 までで探索をうち止めればTLE をふせげそうだが、グラフが密集するほど O(NM)に近づきTLEすることになりそうである。

実際アルゴ式でもある頂点のみの探索になっている。しかし、木であれば十分高速に任意の距離の点の個数を求めることができると判明した点がとてもいい勉強になった。
...
# 275E Sugoroku 4  diff 1264

➀　思考

k 回目マスXに存在する場合、k+1 回目には X+1 ～ X+M のマスに等確率で到達することができる。(ただしNを超える場合は N-(X+M-N) のマスに到達する) したがって、1, 何回目であるか 2, i回目にマスXに到達する場合の数 の二つの情報ですべての遷移を把握できる。さらに、マスの数、出目の数、ルーレットを回す回数はそれぞれ高々 N<=1000 , M<=10 , K<=1000 であるため素直に遷移を処理しても O(NMK) と十分高速である。あとは確率を上手に処理するだけなのだが、回数が異なると同様に確立を考えられないので回数ごとに確率をわけるべきだと考えた。また、i回目にゴールできる場合の数を Mi とすると、その確率は Mi/M^i となるが、確率のMODを最終的に考える必要があるので、分母がM^k となる確率であるほうが都合がよいと考えた。したがって、i回目の処理が終了した時点でカウンターに dp[i+1][-1]* M^(k-(i+1))%MOD　を足し合わせることですべての確立の分子の総和を求められると判断した。最後に、カウンターに分母の逆元をかけてMODをとることで完了　→ AC


➁ 高速化

列を N+1+M に拡張し、imos法を使えば M 回いちいち増やさずとも場合の数を出力できる。累積和を取った後、N 以上のマスに行ってしまった数の分だけ N 以下の対応するマスの場合の数を増やす操作を行えばよい。計算量は O(2(N+M)K) となり O(NMK) から 80%　減少する。

https://twitter.com/H20_dhmo/status/1586352305099902976


# 275C  Yet Another Recursive Function  diff 606

➀ 思考

普通にbfsして累積していくしかなくね。コードテスト　→ TLE → どうにか計算量を減らす工夫を考える　→ 100を例に考えると、100 → 50,33 → 25,16,16,11 となる。ここで 50,33どちらから生成されたかは 16 の遷移に全く影響しない。よって重複して考える必要は全くないのでまとめることができそう　→ ただし、出現回数の情報は、答えに影響しそうなのでCounterで管理することにする。計算量はわからないがこれで素直な実装より十分軽そう → AC

➁ メモ化再帰

工夫の方法として、再帰関数の呼び出しを記録する方法 メモ化再帰がある。これは f(X) の返り値を記録し、以降 f(X) が再び呼び出された場合に、計算なしでその返り値を出力するという方法である。pythonでは @lru_cache(maxsize=None) デコレータを関数の頭につけるだけでメモ化再帰が実装できる。なお、(maxsize=None)はメモ化上限を設定しないことを示していて、default では128以上記録してしまうと古い方から忘れていくようである。
...
1, i段目に到達する場合の数　がわかればすべての遷移を把握できるので 一次元 dp で解答する方針をたてた。あとは +1 ,+2 段目が 集合A　に含まれる場合には遷移を行わない処理を行って AC


# 127D Integer Cards  diff 887
...
書き換える対象は より小さいA になるので A は操作中ずっとソート状態であってほしい。しかしheapq や Sortedset を使ってこれを実現しても、結局書き換えを O(1) で行っていては 最大 O(NM) になるので間に合わない。→ いろいろ考察し、カードをソート状態で集めて最後に大きい順に N 枚の和を取ればよいことに気づく。さらに同じ値でもその枚数を管理しなければいけないのでカウンターを使ったのだが、ソートを維持するためのデータ構造を利用しなくても、最後にカウンターのキーを降順にソートすればそれで十分であることに気が付く。O(M + ( N+M 以下)log(N+M 以下) ) は十分高速なのでこれで完了 → AC


➁ 別解

書き換える値は候補の最大値、対象はA の最小値になる。この操作は Aの最小値が書き換える値以上になった場合、または書き換えがこれ以上行えなくなった場合に終了する、ことを理解し適切に打ち止めることで計算量を削減し、間に合わせることができる。(候補がすべて書き換えられた場合 と A のN枚がすべて書き換わった場合の2通りの終了機会があることに注意)　A,C それぞれソートする必要があり、書き換えが完全に行われると操作回数は最大になるので、総計算量の最大値はO(NlogN + MlogM + max(Bの総和, N)) となる。これは十分高速
